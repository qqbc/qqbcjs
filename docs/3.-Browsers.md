...
Reuse the `api` object for all transactions; it caches ABIs to reduce network usage. Only call `new qqbcjs_api.Api(...)` once.

```html
<pre style="width: 100%; height: 100%; margin:0px; "></pre>

<script src='dist-web/qqbcjs-api.js'></script>
<script src='dist-web/qqbcjs-jsonrpc.js'></script>
<script src='dist-web/qqbcjs-jssig.js'></script>
<script>
  let pre = document.getElementsByTagName('pre')[0];
  const defaultPrivateKey = "5JtUScZK2XEp3g9gh7F8bwtPTRAkASmNrrftmx4AxDKD5K4zDnr"; // bob
  const rpc = new qqbcjs_jsonrpc.JsonRpc('http://localhost:8888');
  const signatureProvider = new qqbcjs_jssig.JsSignatureProvider([defaultPrivateKey]);
  const api = new qqbcjs_api.Api({ rpc, signatureProvider });
  ...
  (async () => {
    try {
      ...
      pre.textContent = '\nCaught exception: ' + e;
      if (e instanceof qqbcjs_jsonrpc.RpcError)
